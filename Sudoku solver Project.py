...
def find_next_empty(puzzle):
    ...
    # return row, col tuple (or (None,None) iff there is none)
    
    # keep in mind that we are using 0-0 for our indexes
    for r in range(9):
        for c in range(9):
            if puzzle[r][c] == -1:
                return r, c
...
def solve_sudoku(puzzle):
    ...
    for guess in range(1, 10): # range(1, 10) is 1, 2, 3, ... 9
        ...
        if is_valid(puzzle, guess, row, col):
            # stp 3.1: if this is valid, then place that guess on the puzzle:
            puzzle[row][col] = guess
            # now recure using this puzzle!
            # step 4: recursively call out function
            if solve_sudoku(puzzle):
                return True
            
        # step 5: if not valid OR if our guess does not solve the puzle, then we need to
